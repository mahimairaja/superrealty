...
describe("tokenSourceForTenant", () => {
  it("posts the tenant slug and agent dispatch, and maps the response", async () => {
    ...
    expect(
      (body.room_config as { agents: { agent_name: string }[] }).agents[0].agent_name,
    ).toBe("realty");
    // No number given: no buyer attribute is sent.
    expect(body.participant_attributes).toBeUndefined();
  });

  it("sends the baked-in number as the buyer.phone attribute, digits only", async () => {
    const { getBody } = mockFetch();

    const source = tokenSourceForTenant("org_abc", "(519) 555-0142");
    await source.fetch({ agentName: "realty" });
  ...
  it("omits the buyer attribute when the number is too short to be real", async () => {
    ...
    const source = tokenSourceForTenant("org_abc", "123");
    await source.fetch({ agentName: "realty" });
